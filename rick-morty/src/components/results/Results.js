...
const Results = (props) => {
  ...
  useEffect(() => {
    ...
    window.history.replaceState(
      page,
      'Updating',
      `search?status=${innerStatus}&gender=${innerGender}&name=${
        name ? name : ''
      }&page=${page}`
    );
  ...
  return error ? (
    ...
    <section style={{ marginBottom: '3rem' }} className="container-fluid">
      ...
      <div
        className={`container-fluid mt-3 d-flex ${
          pageNumber > 1
            ? 'justify-content-end'
      ...
      <div
        className={`container-fluid d-flex wrap flex-wrap pl-0 pr-0  
                ${
                  pageNumber > 1
                    ? 'ml-n3 justify-content-between'
                    : 'ml-n2 justify-content-center'
                }`}
      >
        {data.results.map((ch) => (
